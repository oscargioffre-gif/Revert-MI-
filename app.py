...
def build_calendar_html(df: pd.DataFrame) -> str:
    ...
    # Collect all days present
    day_data = {}  # (month, day) -> pct value
    for dt, val in pct.items():
        day_data[(dt.month, dt.day)] = val

    months_present = sorted(set(dt.month for dt in pct.index))
    ...
    for month in months_present:
        month_pcts = [v for (m, d), v in day_data.items() if m == month]
        ...
        html += f"""
        <tr>
          <td class="month-label">{MONTHS_FULL[month]}</td>
          {cells}

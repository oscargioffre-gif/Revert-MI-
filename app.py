...
<style>
  ...
  /* ── Base ── */
  html, body, [class*="css"] { font-family: 'IBM Plex Sans', sans-serif; background-color: #080a0f; color: #e2e8f0; }
  ...
  div[data-baseweb="select"] { background-color: #161b22 !important; border-color: #30363d !important; border-radius: 8px !important; }
  div[data-baseweb="select"] > div { background-color: #161b22 !important; color: #f0f6fc !important; }
  div[data-baseweb="select"] span { color: #f0f6fc !important; font-weight: 500; font-size: 0.95rem !important; }
  div[data-baseweb="select"] input { color: #f0f6fc !important; caret-color: #3fb950 !important; font-size: 0.95rem !important; }
  div[data-baseweb="select"] svg { fill: #8b949e !important; }
  ul[data-baseweb="menu"] { background-color: #161b22 !important; border: 1px solid #30363d !important; border-radius: 8px !important; }
  ul[data-baseweb="menu"] li { background-color: #161b22 !important; color: #e2e8f0 !important; }
  ul[data-baseweb="menu"] li:hover { background-color: #1f2937 !important; color: #f0f6fc !important; }
  ...
  /* ── Dataframe: forza tema scuro completo ── */
  [data-testid="stDataFrame"] { border-radius: 8px; overflow: hidden; }
  /* Contenitore principale */
  [data-testid="stDataFrame"] > div { background-color: #0d1117 !important; }
  /* Canvas/celle glide data editor */
  [data-testid="stDataFrame"] canvas { filter: none !important; }
  /* Wrapper iframe se presente */
  [data-testid="stDataFrame"] iframe { background: #0d1117 !important; }
  /* Scrollbar */
  [data-testid="stDataFrame"] ::-webkit-scrollbar { background: #161b22; width: 6px; height: 6px; }
  [data-testid="stDataFrame"] ::-webkit-scrollbar-thumb { background: #30363d; border-radius: 3px; }
  /* Forza colori testo su tutti gli elementi interni */
  ...
  ul[data-baseweb="menu"] * { background-color: #161b22 !important; color: #e2e8f0 !important; }
...
# Lista completa: FTSE MIB + mid/small cap Milano con vol medio 3M > 300K
MILAN_TICKERS = [
    # FTSE MIB 40
    ...
    # Mid/Small cap
    "INW.MI","GVS.MI","SOL.MI","MARR.MI","FNM.MI","SAVE.MI","IGD.MI","CIR.MI",
    "TOD.MI","ALERION.MI","SFL.MI","CPR.MI","SFER.MI","SAES.MI","BRE.MI",
    "FILA.MI","SOS.MI","IVG.MI","PIA.MI","TLNT.MI","OVIND.MI","SIEM.MI",
    "RCS.MI","SPM.MI","CASS.MI","ALA.MI","BGN.MI","EQUITA.MI","SECO.MI",
    "ELES.MI","LU.MI","GEDI.MI","IIG.MI","ANIM.MI","BE.MI","CALT.MI",
    "ARIS.MI","CEM.MI","AMBR.MI","ELC.MI","FOPE.MI","IGV.MI","IMMSI.MI",
    "MNL.MI","NB.MI","PN.MI","SIT.MI","UNO.MI","WAY.MI","DEA.MI","MKC.MI",
]
MILAN_TICKERS = list(dict.fromkeys(MILAN_TICKERS))

# FTSE MIB 40 ufficiali: inclusi sempre indipendentemente dal volume
FTSE_MIB_40 = {
    "A2A.MI","AMP.MI","AZM.MI","BAMI.MI","BC.MI","BMED.MI","BMPS.MI","BPER.MI",
    "BZU.MI","CNHI.MI","DIA.MI","ENEL.MI","ENI.MI","ERG.MI","FBK.MI","G.MI",
    "HER.MI","IG.MI","IP.MI","IREN.MI","ISP.MI","LDO.MI","MB.MI","MONC.MI",
...
def load_all(tickers):
    ...
    for ticker in tickers:
        try:
            ...
            # FTSE MIB 40 sempre inclusi; altri richiedono vol medio > 300K
            if avg_vol < 300_000 and ticker not in FTSE_MIB_40: continue
...
with st.sidebar:
    ...
    st.caption("Dati via yfinance · Vol medio 3M > 300K · 365 gg")
    if st.button("🔄  Aggiorna Dati", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
    st.divider()
    rsi_min, rsi_max = st.slider("Filtro RSI", 0, 100, (0, 100))
    vol_min = st.slider("Vol. Medio min (M)", 0.3, 10.0, 0.3, step=0.1)
    st.divider()
...
for col, label, val, sub, cls in [
    (c1, "Titoli Attivi", str(n_t), f"Vol media 3M >{vol_min}M", ""),

...
def make_chart(df: pd.DataFrame, ticker: str) -> go.Figure:
    ...
    # Volume bars — rgba for plotly 6.x compat
    vol_colors = [
        "rgba(63,185,80,0.25)" if v >= 0 else "rgba(248,81,73,0.25)"
        for v in df["pct"].fillna(0)
    ]
    ...
    fig.add_trace(go.Scatter(
        ...
        fillcolor="rgba(210,153,34,0.13)", name="ATR %"
    ), row=3, col=1)
